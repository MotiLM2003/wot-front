...
const Stage2 = ({
	...
	return (
		...
		>
			<div className='flex flex-col gap-2'>
				...
				<div>
					<Input
						...
						name='companyName'
						onChange={onChange}
						className={errors?.companyName ? `border-2 border-red` : ''}
					/>
					<div className='flex gap-3 mt-4 '>
						<textarea
							...
							className={`${
								errors?.moreDetails ? `border-2 border-red` : ''
							} bg-white outline-none min-h-[100px] text-xs w-[100%] border rounded p-2  border-shades-300`}
						/>
					</div>
				</div>
			</div>
			...
			{isHasErrors && (
				<div className='mt-[1rem] gap-2 flex justify-center bg-white text-red p-[.5rem] rounded border-[.5px] border-red'>
					Please address the errors highlighted in red and try again.
				</div>
			)}
