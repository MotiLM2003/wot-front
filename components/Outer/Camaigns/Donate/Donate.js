...
const Donate = ({
	...
	// DONATE FORM USE STATE
	const [errors, setErrors] = useState(setTempErros());

	function setTempErros() {
		const tempErrors = {};
		for (const prop in initialRecurringData) {
	...
	function validate() {
		...
		tempErrors.displayName = !minLength(recurring.displayName, 3);
		tempErrors.sum = recurring.sum < 1;
		tempErrors.email = !email(recurring.email);
		tempErrors.firstName = !minLength(recurring.firstName, 3);
		tempErrors.lastName = !minLength(recurring.lastName, 3);
		tempErrors.cellphone = !minLength(recurring.cellphone, 3);
		tempErrors.isAgreeToTerms = !recurring.isAgreeToTerms;
